...
| Path | Description |
|------|-------------|
| [`api/MyGpsTracker.Api`](api/MyGpsTracker.Api) | ASP.NET Core 9 backend — CQRS/MediatR + Dapper, TCP ingestion, SignalR |
| [`db`](db) | PostGIS schema (Docker init) |
| [`web`](web) | React 18 + TypeScript + Vite + Tailwind dashboard (maps via Leaflet) |
| [`web/android`](web/android) | Capacitor hybrid Android app (biometric auth, secure storage) |
| [`gps-simulator`](gps-simulator) | Standalone Android fleet simulator (Teltonika Codec 8 over TCP) |
...
- Threshold-based alerts: speeding, hard_brake, hard_curve, low_battery, offline, geofence_speeding
